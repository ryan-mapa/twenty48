...
export async function submitRun(sessionId, run) {
    return parse(await fetch(`/session/${sessionId}/submit`, {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        // Only the move log and timings are sent. The server derives the
        // score itself; there is deliberately nothing here to inflate.
        body: JSON.stringify({ moves: run.moves, timings: run.timings })
    }));
}
